...
wasm::Expression EWasmCodeTransform::operator()(FunctionCall const& _call)
{
	...
	{
		static set<YulString> const untyped{"drop"_yulstring, "unreacheable"_yulstring};
		string prefix = "i64.";
		if (untyped.count(_call.functionName.name))
			prefix.clear();
		return wasm::BuiltinCall{prefix + _call.functionName.name.str(), visit(_call.arguments)};
	}
	else
...
wasm::Expression EWasmCodeTransform::operator()(ForLoop const& _for)
{
	...
	conditionCheck.arguments.emplace_back(std::move(loopCondition));

	wasm::Loop loop;
	loop.statements.emplace_back(std::move(conditionCheck));
	...
	breakBlock.statements.emplace_back(std::move(loop));
