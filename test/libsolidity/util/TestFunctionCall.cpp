...
namespace
{
	...
	/// soltest.
	auto getContractABITypes(string const& _type) -> vector<ABIType>
	{
		regex boolType{"(bool)"};
		regex unsignedType{"(uint\\d*)"};
		regex signedType{"(int\\d*)"};
		regex bytesType{"(bytes\\d+)"};
		regex dynBytesType{"(\\bbytes\\b)"};

		vector<ABIType> abiTypes;
		if (regex_match(_type, boolType))
			abiTypes.push_back(ABIType{ABIType::Boolean, ABIType::AlignRight, 32});
		else if (regex_match(_type, unsignedType))
			abiTypes.push_back(ABIType{ABIType::UnsignedDec, ABIType::AlignRight, 32});
		else if (regex_match(_type, signedType))
			abiTypes.push_back(ABIType{ABIType::SignedDec, ABIType::AlignRight, 32});
		else if (regex_match(_type, bytesType))
			abiTypes.push_back(ABIType{ABIType::Hex, ABIType::AlignRight, 32});
		else if (regex_match(_type, dynBytesType))
		{
			abiTypes.push_back(ABIType{ABIType::UnsignedDec, ABIType::AlignRight, 32});
			abiTypes.push_back(ABIType{ABIType::UnsignedDec, ABIType::AlignRight, 32});
...
) const
{
	...
	/// Infere type from Contract ABI. Used to generate values for
	...
	for (auto const& function: m_contractABI)
		if (function["name"] == functionName)
			...
			{
				auto types = getContractABITypes(output["type"].asString());
